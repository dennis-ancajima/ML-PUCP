...
%% 8. Aplicar Random Forest con validación cruzada k=10
...
for i = 1:cv.NumTestSets
    ...
    rfModel = TreeBagger(100, XTrainCV, YTrainCV);  % Entrenar el modelo de Random Forest con 100 árboles
    
    YTestPredicted = predict(rfModel, XTestCV);  % Realizar la predicción
    YTestPredicted = str2double(YTestPredicted);  % Convertir las etiquetas predichas a números
...
disp(['Exactitud promedio con validación cruzada (Random Forest): ', num2str(meanAccuracy*100), '%']);
S
% accuracy == 0.9607
