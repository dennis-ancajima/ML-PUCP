...
trainRatio = 0.8;  % Proporción de datos para entrenamiento
...
%% 8. Aplicar ANN con validación cruzada k=10
...
for i = 1:cv.NumTestSets
    ...
    annModel = fitnet([10 10]);  % Crear modelo de Red Neuronal Artificial (ANN)
    annModel = train(annModel, XTrainCV', YTrainCV');  % Entrenar la ANN
    
    YTestPredicted = round(annModel(XTestCV'));  % Realizar la predicción
...
disp(['Exactitud promedio con validación cruzada (ANN): ', num2str(meanAccuracy*100), '%']);
